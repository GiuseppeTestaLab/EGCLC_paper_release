...
output:
    html_document:
        ...
        code_folding: hide
params: 
    InputFolder: '~/EGCLCclones/1.Preprocessing/Output/methyldackel'
    OutputFolder: '~/EGCLCclones/2.PreliminaryAnalysis/'
---
...
```{r, collapse=TRUE}
sample_anno <- data.frame("SampleID" = c("hiPSC_rep5", "hEGCLC_p10_rep1", "hEGCLC_p10_rep2", "hEGCLC_p10_rep3"), "OriginalID" = c("hiPSC", "hEGCLC_clone2", "hEGCLC_clone9", "hEGCLC_clone31"), "Type" = c("hiPSCs", "hEGCLCs_p10", "hEGCLCs_p10", "hEGCLCs_p10"), "Line" = rep("CTL08A", 4), "passage" = c("26", "10", "10", "10"))
sample_anno$Type <- factor(sample_anno$Type)
...
Adding previous experiment samples 

```{r, collapse=TRUE}
bdg_files2 <-  list.files(path = "~/DataDir/0.Preprocessing/Output", pattern = '*\\.bedGraph', full.names = TRUE) #output from our preprocessing
```

```{r, collapse=TRUE}
bdg_files_filtered2 <- bdg_files2[grep("filtered", bdg_files2)]
```

```{r, collapse=TRUE}
sample_anno2 <- read.table("~/DataDir/1.PreliminaryAnalysis/Input/SampleAnno.txt", sep='\t', header=TRUE)

sample_anno2[sample_anno2$Type %in% "hEGCLCs", "Type"] <- "hEGCLCs_p5"
sample_anno2$SampleID <- sub("hEGCLC_", "hEGCLC_p5_", sample_anno2$SampleID)
rownames(sample_anno2) <- sample_anno2$SampleID
```

Keeping only bedGraph files of selected samples.

```{r, collapse=TRUE}
bdg_files_filtered2 <- bdg_files_filtered2[which(grepl(paste(sample_anno2$OriginalID, collapse = "|"), bdg_files_filtered2) )]
```

Match order

```{r, collapse=TRUE}
bdg_files_filtered2 <- bdg_files_filtered2[order(match(sub(".filtered.bedGraph", "", basename(bdg_files_filtered2)), sample_anno2$OriginalID))]
```

```{r}
sample_anno <- plyr::rbind.fill(sample_anno2, sample_anno)
rownames(sample_anno) <- sample_anno$SampleID
bdg_files_filtered <- c(bdg_files_filtered2, bdg_files_filtered)

sample_anno <- rbind(sample_anno[1:4,], sample_anno[15,], sample_anno[5:14,], sample_anno[16:18,])
```
