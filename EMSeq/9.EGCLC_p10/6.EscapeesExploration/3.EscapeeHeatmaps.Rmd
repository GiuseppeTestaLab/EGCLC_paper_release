...
output:
  ...
  pdf_document:
    toc: yes
params:
  InputFolder: '~/EGCLCclones/3.EscapeesExploration/'
---
...
```{r}
AnnotatedEscapees <- readRDS(paste0(InputFolder, "AnnotatedEscapees.rds"))
```
...
colData(SE)$Type <- factor(colData(SE)$Type, levels = c("hiPSCs", "iMeLCs", "hPGCLCs", "hEGCLCs_p5", "hEGCLCs_p10"))
...
```{r}
#break_coord <- assay(SE) %>% quantile(seq(0,1, by=0.05), na.rm =TRUE)
#ExpCols <- viridisLite::plasma(length(seq(break_coord[2], tail(break_coord,1), by=0.05)))
ExpCols <- viridisLite::plasma(length(seq(0.8, 1, by=0.05)))
sechm(SE, features = rownames(SE), hmcols=ExpCols, breaks=seq(0.8, 1, by=0.05), show_rownames=FALSE, show_colnames=TRUE, do.scale=FALSE, column_title = "Median methylation levels (by gene) of CpGs in escapee regions", top_annotation = c("Type"), row_title_rot = 0, cluster_rows = TRUE)
```
