---
title: "Heatmaps of Murase et al. escapees CpG methylation levels"
author: "Gaja Matassa"
...
output:
  ...
  pdf_document:
    toc: yes
params:
  InputFolder: '~/DataDir/7.EscapeesExploration/Escapees_Murase/Output/'
---
...
```{r}
bsseq_obj_escapees <- readRDS(paste0(InputFolder, "bsseq_obj_escapees_Murase.rds"))
```

Loading annotated escapees
```{r}
AnnotatedEscapees <- readRDS("~/DataDir/7.EscapeesExploration/Escapees_Murase/Output/EscapeesAnnotationAndFunctional/AnnotatedEscapees.rds")
```
...
In total we have `r nrow(SE)` average meth levels. Too many for an heatmap. I'll plot the first 5000.
...
sechm(SE, features = rownames(SE)[1:5000], hmcols=ExpCols, breaks=seq(head(break_coord,1), tail(break_coord,1), by=0.2), show_rownames=FALSE, show_colnames=TRUE, do.scale=FALSE, column_title = "Average methylation levels (by gene) of CpGs in escapee regions", top_annotation = c("Type"), row_title_rot = 0, cluster_rows = TRUE)
```
...
sechm(SE, features = rownames(SE)[1:5000], hmcols=ExpCols, breaks=seq(break_coord[2], tail(break_coord,1), by=0.05), show_rownames=FALSE, show_colnames=TRUE, do.scale=FALSE, column_title = "Average methylation levels (by gene) of CpGs in escapee regions", top_annotation = c("Type"), row_title_rot = 0, cluster_rows = TRUE)
```
...
break_coord <- assay(SE) %>% quantile(seq(0,1, by=0.1), na.rm =TRUE)
ExpCols <- viridisLite::plasma(length(seq(head(break_coord,1), tail(break_coord,1), by=0.2)))
sechm(SE, features = rownames(SE)[1:5000], hmcols=ExpCols, breaks=seq(head(break_coord,1), tail(break_coord,1), by=0.2), show_rownames=FALSE, show_colnames=TRUE, do.scale=FALSE, column_title = "Median methylation levels (by gene) of CpGs in escapee regions", top_annotation = c("Type"), row_title_rot = 0, cluster_rows = TRUE)
```
...
break_coord <- assay(SE) %>% quantile(seq(0,1, by=0.1), na.rm =TRUE)
ExpCols <- viridisLite::plasma(length(seq(break_coord[2], tail(break_coord,1), by=0.05)))
sechm(SE, features = rownames(SE)[1:5000], hmcols=ExpCols, breaks=seq(break_coord[2], tail(break_coord,1), by=0.05), show_rownames=FALSE, show_colnames=TRUE, do.scale=FALSE, column_title = "Median methylation levels (by gene) of CpGs in escapee regions", top_annotation = c("Type"), row_title_rot = 0, cluster_rows = TRUE)
```
