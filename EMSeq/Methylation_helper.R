...
plot_PCA <- function(pca_res, anno = NULL, col_anno = NULL, shape_anno = NULL,
                     ...
                     custom_colors = NULL, point_size = 4, interactive=FALSE) {
...
DMRviolin <- function(bsseq_obj, assayName = "Meth", title = "Methylation Levels in DMRs", 
  ...
  samples <- colnames(bsseq_obj) 
  
  methylation_data <- as.data.frame(assays(bsseq_obj)[[assayName]])
  colnames(methylation_data) <- colnames(bsseq_obj)
  methylation_data$GenomicRegion <- rowData(bsseq_obj)$GenomicRegion
  methylation_data$Locus <- rownames(methylation_data)
  
  methylation_long <- methylation_data %>%
    pivot_longer(cols = -c(GenomicRegion, Locus), 
                 names_to = "Sample",
                 values_to = "MethylationLevel")
  
  methylation_long$Sample <- factor(methylation_long$Sample, levels = samples)
  
  if (fill == "Genomic region") {
    p <-   ggplot(methylation_long, aes(x = GenomicRegion, y = MethylationLevel, fill = GenomicRegion)) +
      facet_wrap(~ Sample, scales = "free_y") +
      labs(x = "Genomic Region", y = "Methylation Level") +
      scale_fill_manual(values = metadata(bsseq_obj)$anno_colors$GenomicRegion)
  } else{
    p <- ggplot(methylation_long, aes(x = Sample, y = MethylationLevel, fill = Sample)) +
      facet_wrap(~ GenomicRegion, scales = "free_y") + 
      labs(x = "Sample", y = "Methylation Level") +
   ...
   p <- p + geom_violin(trim = TRUE) +
    ...
    guides(fill = "none") +
    stat_summary(fun = stat, geom = "crossbar", width = 0.5,colour = "red")
